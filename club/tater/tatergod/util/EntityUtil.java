...
import club.tater.tatergod.mixin.mixins.IEntityLivingBase;
...
public class EntityUtil implements Util {
    ...
    public static double getBaseMoveSpeed() {
        ...
        if (EntityUtil.mc.player != null && EntityUtil.mc.player.isPotionActive(Potion.getPotionById(1))) {
            int amplifier = EntityUtil.mc.player.getActivePotionEffect(Potion.getPotionById(1)).getAmplifier();
    ...
    public static boolean isntValid(Entity entity, double range) {
        return entity == null || isDead(entity) || entity.equals(EntityUtil.mc.player) || entity instanceof EntityPlayer && Tater.friendManager.isFriend(entity.getName()) || EntityUtil.mc.player.getDistanceSq(entity) > MathUtil.square(range);
    }
    ...
    public static Color getColor(Entity entity, int red, int green, int blue, int alpha, boolean colorFriends) {
        ...
        if (entity instanceof EntityPlayer && colorFriends && Tater.friendManager.isFriend((EntityPlayer) entity)) {
    ...
    public static boolean isCrystalAtFeet(EntityEnderCrystal crystal, double range) {
        ...
        while (iterator.hasNext()) {
            ...
            if (EntityUtil.mc.player.getDistanceSq(player) <= range * range && !Tater.friendManager.isFriend(player)) {
                Vec3d[] avec3d = EntityUtil.doubleLegOffsetList;
                int i = avec3d.length;
    ...
    public static Map getTextRadarPlayers() {
        ...
        while (iterator.hasNext()) {
            ...
            if (!player.isInvisible() && !player.getName().equals(EntityUtil.mc.player.getName())) {
                ...
                ((Map) output).put(healthSB + " " + (Tater.friendManager.isFriend(player) ? ChatFormatting.AQUA : ChatFormatting.RED) + player.getName() + " " + distanceSB + " Â§f0", Integer.valueOf((int) EntityUtil.mc.player.getDistance(player)));
