...
public class GuiBetterChat extends GuiNewChat {
    ...
    public static void updatePercentage(long diff) {
        if (GuiBetterChat.percentComplete < 1.0F) {
            GuiBetterChat.percentComplete += 0.004F * (float) diff;
        }
    ...
    public void drawChat(int updateCounter) {
        if (!this.configuring) {
            if (GuiBetterChat.prevMillis == -1L) {
                ...
                long diff = current - GuiBetterChat.prevMillis;
                ...
                if (this.mc.gameSettings.chatVisibility != EnumChatVisibility.HIDDEN) {
                    ...
                    if (j > 0) {
                        ...
                        if (BetterChat.getSettings().smooth && !this.isScrolled) {
                            GlStateManager.translate(2.0F + (float) BetterChat.getSettings().xOffset, 8.0F + (float) BetterChat.getSettings().yOffset + (9.0F - 9.0F * percent) * f1, 0.0F);
                        } else {
                            GlStateManager.translate(2.0F + (float) BetterChat.getSettings().xOffset, 8.0F + (float) BetterChat.getSettings().yOffset, 0.0F);
                        }
                            ...
                            if (l2 != null) {
                                ...
                                if (i3 < 200 || flag) {
                                    ...
                                    if (k3 > 3) {
                                        ...
                                        if (BetterChat.getSettings().smooth && k2 <= GuiBetterChat.newLines) {
                                            this.mc.fontRenderer.drawStringWithShadow(s, 0.0F, (float) (j2 - 8), 16777215 + ((int) ((float) k3 * percent) << 24));
                                        } else {
    ...
    public void printChatMessageWithOptionalDeletion(ITextComponent chatComponent, int chatLineId) {
        ...
        GuiBetterChat.LOGGER.info("[CHAT] {}", chatComponent.getUnformattedText().replaceAll("\r", "\\\\r").replaceAll("\n", "\\\\n"));
    }
