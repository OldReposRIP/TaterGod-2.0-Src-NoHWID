...
public class RenderUtil implements Util {
    ...
    public static RenderItem itemRender = RenderUtil.mc.getRenderItem();
    ...
    public static void drawGradientRect(int x, int y, int w, int h, int startColor, int endColor) {
        ...
        vertexbuffer.pos((double) x + (double) w, (double) y, 0.0D).color(f2, f3, f4, f).endVertex();
        vertexbuffer.pos((double) x, (double) y, 0.0D).color(f2, f3, f4, f).endVertex();
        vertexbuffer.pos((double) x, (double) y + (double) h, 0.0D).color(f6, f7, f8, f5).endVertex();
        vertexbuffer.pos((double) x + (double) w, (double) y + (double) h, 0.0D).color(f6, f7, f8, f5).endVertex();
        tessellator.draw();
    ...
    public static void drawText(BlockPos pos, String text) {
        if (pos != null && text != null) {
            ...
            GlStateManager.disableDepth();
            GlStateManager.translate(-((double) Tater.textManager.getStringWidth(text) / 2.0D), 0.0D, 0.0D);
            Tater.textManager.drawStringWithShadow(text, 0.0F, 0.0F, -5592406);
            GlStateManager.popMatrix();
        }
    }
    ...
    public static void drawClock(float x, float y, float radius, int slices, int loops, float lineWidth, boolean fill, Color color) {
        ...
        if (fill) {
            ...
            for (int color2 = 0; color2 <= 360; ++color2) {
                Vec2f j = new Vec2f(x + (float) Math.sin((double) color2 * 3.141592653589793D / 180.0D) * radius, y + (float) Math.cos((double) color2 * 3.141592653589793D / 180.0D) * radius);
